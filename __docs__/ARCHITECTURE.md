# SWEEP Game - アーキテクチャドキュメント

## 概要

SWEEP Gameは、TypeScript + PixiJS + PWAで構築された高性能な60FPS Webマインスイーパーゲームです。モジュラー設計により機能が明確に分離され、各システムが独立して動作する設計となっています。
...
この設計により、SWEEP Gameは高いパフォーマンスと保守性を両立した、スケーラブルなWebゲームとなっています。